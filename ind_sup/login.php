...
$sql = "SELECT * FROM `users` WHERE `email`=? AND `password`=? AND `status`=? AND `groupname`!=? AND `groupname`!=?";
			$query = $dbh->prepare($sql);
			$query->execute(array($email,$password,$status,'Siwes Coordinator','Institution Supervisor'));
			$row = $query->rowCount();
			$fetch = $query->fetch();
			if($row > 0) {
...
<title>Industry Supervisor Login|<?php echo $row_website['website_name'];   ?></title>
    <!-- plugins:css -->
